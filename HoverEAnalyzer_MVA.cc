...
private:
  ...
  edm::EDGetTokenT<edm::View<reco::PFCandidate> >pfCandidateCollection_;
  //edm::EDGetTokenT<edm::View<reco::Photon> >endcapPhotonCollection_;
...
HoverEAnalyzer_MVA::HoverEAnalyzer_MVA(const edm::ParameterSet& iConfig):
  ...
  pfCandidateCollection_(consumes <edm::View<reco::PFCandidate> >(iConfig.getUntrackedParameter<edm::InputTag>("pfCandidateCollection",edm::InputTag("particleFlow")))),
  //endcapPhotonCollection_(consumes <edm::View<reco::Photon> >(iConfig.getUntrackedParameter<edm::InputTag>("endcapPhotonCollection",edm::InputTag("photons")))),
...
{ 
  ...
  /*Mva_.reset( new TMVA::Reader( "!Color:Silent" ) );
    ...
    Mva_->BookMVA( "BDTG",MVAweightfile_.fullPath() );*/ // commented out from the instruction of Louie
  ...
  treePhoton->Branch("chargedHadronIso"           ,&infoPhoton.chargedHadronIso          ,"chargedHadronIso/F");
  treePhoton->Branch("neutralHadronIso"           ,&infoPhoton.neutralHadronIso          ,"neutralHadronIso/F");
  treePhoton->Branch("photonIso"           ,&infoPhoton.photonIso          ,"photonIso/F");
  ...
  treeBackground->Branch("chargedHadronIso"           ,&infoBackground.chargedHadronIso          ,"chargedHadronIso/F");
  treeBackground->Branch("neutralHadronIso"           ,&infoBackground.neutralHadronIso          ,"neutralHadronIso/F");
  treeBackground->Branch("photonIso"           ,&infoBackground.photonIso          ,"photonIso/F");
  cellSize_ = 1;
  doLogWeight_ = true;
  mipE_ = 0.0000551;
  nSR_ = 5;
...
{	
  ...
  Handle<edm::View<reco::PFCandidate> > PFCands;
  iEvent.getByToken(pfCandidateCollection_,PFCands);
  const PtrVector<reco::PFCandidate>& pcands = PFCands->ptrVector();
  ...
  float photonPrimaryVertexZ = -999.0;
  float photonPrimaryVertexX = -999.0;
  float photonPrimaryVertexY = -999.0;
  ...
  for (unsigned int igp =0; igp < gens.size() ; igp++) { // loop over gen particles to fill truth-level tree
    ...
    infoPhoton.chargedHadronIso = 0;
    infoPhoton.neutralHadronIso = 0;
    infoPhoton.photonIso = 0;
    ...
    photonPrimaryVertexZ = vPos.z();
    photonPrimaryVertexX = vPos.x();
    photonPrimaryVertexY = vPos.y();
    ...
    if (infoPhoton.matchIndex >-1) {
      ...
      infoPhoton.chargedHadronIso = 0;
      infoPhoton.photonIso = 0;
      for(unsigned int ipfc = 0; ipfc < pcands.size(); ipfc++){
	float drparticle = dRCalc(sclusters[infoPhoton.matchIndex]->eta(), sclusters[infoPhoton.matchIndex]->phi(), pcands[ipfc]->eta(), pcands[ipfc]->phi());
	if(drparticle < 0.3){
	  float deltaEta = std::fabs(sclusters[infoPhoton.matchIndex]->eta() - pcands[ipfc]->eta());
	  if(pcands[ipfc]->particleId() == pcands[ipfc]->h0)infoPhoton.neutralHadronIso += pcands[ipfc]->energy();
	  if(drparticle > 0.02 && deltaEta > 4*0.00864*std::fabs(sinh(photonFoundSCEta)) && pcands[ipfc]->particleId() == pcands[ipfc]->gamma){
	    infoPhoton.photonIso += pcands[ipfc]->energy();
	  } 
	  if(drparticle > 0.02 && pcands[ipfc]->particleId() == pcands[ipfc]->h){
	    float dz = std::abs(pcands[ipfc]->vz() - photonPrimaryVertexZ);
	    float dx = std::fabs(pcands[ipfc]->vx() - photonPrimaryVertexX);
	    float dy = std::fabs(pcands[ipfc]->vy() - photonPrimaryVertexY);
	    float dxy = sqrt(dx*dx+dy*dy);
	    if(dz < 0.2 && dxy < 0.1)infoPhoton.chargedHadronIso += pcands[ipfc]->energy();
	  }
	}
      }
      //std::cout << "NeutralHadisoSig:" << infoPhoton.neutralHadronIso << std::endl;
      //std::cout << "ChargedHadisoSig:" << infoPhoton.chargedHadronIso << std::endl;
      //std::cout << "PhotonIsoSig:" << infoPhoton.photonIso << std::endl;
      ...
      infoPhoton.MVA = 0;// instruction from Louie
  ...
  if(photonFound){ //only want SCs which are NOT the photon.
    ...
    for( unsigned int isc =0; isc< sclusters.size() ; isc++){ // isc = index_super_cluster
      ...
      infoBackground.neutralHadronIso = 0;
      infoBackground.chargedHadronIso = 0;
      infoBackground.photonIso = 0;
      ...
      infoBackground.photonIso = 0;
      for(unsigned int ipfc = 0; ipfc < pcands.size(); ipfc++){
	float drparticle = dRCalc(sclusters[isc]->eta(), sclusters[isc]->phi(), pcands[ipfc]->eta(), pcands[ipfc]->phi());
	if(drparticle < 0.3){
	  float deltaEta = std::fabs(sclusters[isc]->eta() - pcands[ipfc]->eta());
	  if(pcands[ipfc]->particleId() == pcands[ipfc]->h0)infoBackground.neutralHadronIso += pcands[ipfc]->energy();
	  if(drparticle > 0.02 && deltaEta > 4*0.00864*std::fabs(sinh(photonFoundSCEta)) && pcands[ipfc]->particleId() == pcands[ipfc]->gamma){
	    infoBackground.photonIso += pcands[ipfc]->energy();
	  } 
	  if(drparticle > 0.02 && pcands[ipfc]->particleId() == pcands[ipfc]->h){
	    float dz = std::abs(pcands[ipfc]->vz() - photonPrimaryVertexZ);
	    float dx = std::fabs(pcands[ipfc]->vx() - photonPrimaryVertexX);
	    float dy = std::fabs(pcands[ipfc]->vy() - photonPrimaryVertexY);
	    float dxy = sqrt(dx*dx+dy*dy);
	    if(dz < 0.2 && dxy < 0.1)infoBackground.chargedHadronIso += pcands[ipfc]->energy();
	  }
	}
      }
      //std::cout << "NeutralHadisoBKG:" << infoBackground.neutralHadronIso << std::endl;
      //std::cout << "ChargedHadisoBKG:" << infoBackground.chargedHadronIso << std::endl;
      //std::cout << "PhotonIsoBKG:" << infoBackground.photonIso << std::endl;
      // fill ID vars
      ...
      infoBackground.MVA = 0;  // instructions from Louie
      ///---------> Isolation avraibles test <-------------
