...
double HGCALShowerBasedEmIdentificationLC2::lengthCompatibilityElectron(const reco::PFCluster& clu,const  edm::PtrVector<HGCRecHit>& rcs  ){
	if (!showerPosIsSet_) {
		std::cout << "[HGCALShowerBasedEmIdentificationLC2::lengthCompatibility] error, showwer position not set " << std::endl;
		std::cout << "[HGCALShowerBasedEmIdentificationLC2::lengthCompatibility] error, please invoke setShowerPos and setShowerDir before invoking this function " << std::endl;
		return 0.;
	...
	double lengthCompatibilityElectron=0., predictedLength=0., predictedSigma=0.;
	...
	predictedSigma = predictedLength*sqrt(predictedSigma);
	predictedSigma = sqrt( predictedSigma*predictedSigma + radiationLength_*radiationLength_);
	radiationLength_= radiationLength_ ; //return radition length to the original value

	lengthCompatibilityElectron = (predictedLength-length)/predictedSigma;
	
	
	return lengthCompatibilityElectron;
